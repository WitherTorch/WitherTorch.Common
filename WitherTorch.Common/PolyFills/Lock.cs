...
using WitherTorch.Common.PolyFills;
...
public sealed partial class Lock
{
    ...
    private AutoResetEventLite? WaitEvent
    {
        get
        {
            ...
            return (AutoResetEventLite?)weoc;
        }
    }
    ...
    public void Exit()
    {
        ...
        [MethodImpl(MethodImplOptions.NoInlining)]
        static void Throw() => throw new SynchronizationLockException("The calling thread does not hold the lock.");
    }
    ...
    internal int TryEnterSlow(int timeoutMs, int currentThreadId)
    {
        ...
        else if (_owningThreadId == currentThreadId)
        {
            ...
            throw new LockRecursionException("The lock has reached the limit of recursive enters.");
        }
    ...
    Wait:
        AutoResetEventLite waitEvent = WaitEvent ?? CreateWaitEvent();
        ...
        try
        {
            ...
            while (true)
            {
                if (!waitEvent.Wait(remainingTimeoutMs))
                {
                    break;
                }
    ...
    [MethodImpl(MethodImplOptions.NoInlining)]
    private AutoResetEventLite CreateWaitEvent()
    {
        var newWaitEvent = new AutoResetEventLite();
        ...
        return (AutoResetEventLite)weocBeforeUpdate;
    }
    ...
    private void SignalWaiterIfNecessary(State state)
    {
        if (State.TrySetIsWaiterSignaledToWake(this, state))
        {
            ...
            AutoResetEventLite? waitEvent = WaitEvent;
            DebugHelper.ThrowUnless(waitEvent != null);
            waitEvent.Set();
        }
    }
    ...
    private static bool TryInitializeStatics()
    {
        ...
        try
        {
            if (oldStage == StaticsInitializationStage.NotStarted)
            {
                ...
                s_isSingleProcessor = Environment.ProcessorCount == 1;
    ...
    private static short DetermineMinSpinCountForAdaptiveSpin()
    {
        ...
        short adaptiveSpinPeriod = DefaultAdaptiveSpinPeriod;
...
#pragma warning restore CS9216 // A value of type 'System.Threading.Lock' converted to a different type will use likely unintended monitor-based locking in 'lock' statement.
    ...
    private struct State : IEquatable<State>
    {
        ...
        public static bool TryLockBeforeWait(Lock lockObj)
        {
            ...
            [MethodImpl(MethodImplOptions.NoInlining)]
            static void Throw() => throw new OutOfMemoryException("The lock has reached the limit of how many threads may wait for the lock.");
        }
