...
namespace WitherTorch.Common.Text
{
    unsafe partial class AsciiEncodingHelper
    {
        ...
        internal static byte* ReadFromUtf16BufferCore_OutOfAsciiRange(char* source, byte* destination, nuint length)
        {
            if (Limits.CheckTypeCanBeVectorized<ushort>() && Limits.CheckTypeCanBeVectorized<byte>() && length > Limits.GetLimitForVectorizing<byte>())
                return VectorizedReadFromUtf16BufferCore_OutOfAsciiRange(source, destination, length);
            return ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(ref source, ref destination, ref length);
        }
        ...
        private static byte* ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(ref char* source, ref byte* destination, ref nuint length)
        {
            for (; length >= 4; length -= 4, source += 4, destination += 4)
            {
                destination[0] = ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(source[0]);
                destination[1] = ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(source[1]);
                destination[2] = ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(source[2]);
                destination[3] = ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(source[3]);
            }
            char* sourceEnd = source + length;
            byte* destinationEnd = destination + length;
            if (source >= sourceEnd)
                goto Result;
            *destination++ = ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(*source++);
            if (source >= sourceEnd)
                goto Result;
            *destination++ = ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(*source++);
            if (source >= sourceEnd)
        ...
        [Inline(InlineBehavior.Remove)]
        private static byte ScalarizedReadFromUtf16BufferCore_OutOfAsciiRange(char source)
        {
            const byte ReplaceCharacter = 0x003F;
#if NET8_0_OR_GREATER
            return source > AsciiEncodingLimit ? ReplaceCharacter : unchecked((byte)source);
#else
            nuint mask = unchecked((nuint)(-Helpers.MathHelper.BooleanToNative(source > AsciiEncodingLimit)));
