...
namespace WitherTorch.Common.Extensions
{
    unsafe partial class VectorExtensions
    {
        ...
        private static ulong ExtractMostSignificantBits_64<T>(in Vector<T> vector) where T : struct
            => ExtractMostSignificantBitsCore<T>(*(ulong*)UnsafeHelper.AsPointerIn(in vector));

        [Inline(InlineBehavior.Remove)]
        private static ulong ExtractMostSignificantBits_128<T>(in Vector<T> vector) where T : struct
            => ExtractMostSignificantBitsCore_128<T>((ulong*)UnsafeHelper.AsPointerIn(in vector), sizeof(ulong) / sizeof(T));

        [Inline(InlineBehavior.Remove)]
        private static ulong ExtractMostSignificantBits_256<T>(in Vector<T> vector) where T : struct
            => ExtractMostSignificantBitsCore_256<T>((ulong*)UnsafeHelper.AsPointerIn(in vector), sizeof(ulong) / sizeof(T));

        [Inline(InlineBehavior.Remove)]
        private static ulong ExtractMostSignificantBits_512<T>(in Vector<T> vector) where T : struct
            => ExtractMostSignificantBitsCore_512<T>((ulong*)UnsafeHelper.AsPointerIn(in vector), sizeof(ulong) / sizeof(T));

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong ExtractMostSignificantBitsCore_128<T>(ulong* vector, int bitCountPer64) where T : struct
            => ExtractMostSignificantBitsCore<T>(vector[0]) |
            (ExtractMostSignificantBitsCore<T>(vector[1]) << bitCountPer64);

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong ExtractMostSignificantBitsCore_256<T>(ulong* vector, int bitCountPer64) where T : struct
            => ExtractMostSignificantBitsCore<T>(vector[0]) |
            (ExtractMostSignificantBitsCore<T>(vector[1]) << bitCountPer64) |
        ...
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        private static ulong ExtractMostSignificantBitsCore_512<T>(ulong* vector, int bitCountPer64) where T : struct
            => ExtractMostSignificantBitsCore<T>(vector[0]) |
            (ExtractMostSignificantBitsCore<T>(vector[1]) << bitCountPer64) |
            (ExtractMostSignificantBitsCore<T>(vector[2]) << (bitCountPer64 * 2)) |
            (ExtractMostSignificantBitsCore<T>(vector[3]) << (bitCountPer64 * 3)) |
